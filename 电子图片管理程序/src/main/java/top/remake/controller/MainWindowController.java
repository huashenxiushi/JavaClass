...
/**
 初始化文件树视图(initFileTreeView)：这个方法用于设置文件树视图的单元格工厂，并添加文件系统的根目录到树视图中。
 初始化预览流面板(initPreviewFlowPane)：这个方法用于初始化预览流面板，并添加鼠标事件监听器。
 初始化搜索功能(initSearch)：这个方法用于添加搜索框的监听事件，实现搜索内容的高亮显示。
 初始化排序面板(initSortPane)：这个方法用于初始化排序选择框，并添加监听器。
 处理图片的复制、粘贴、删除、重命名等操作。
 实现图片的预览和播放幻灯片功能。
 实现导航到指定路径的功能。
 实现图片的压缩和属性查看功能。
 实现撤销和重做功能。
 这个类的主要作用是作为主窗口的控制器，处理用户在主窗口中的各种操作，并更新视图以反映这些操作的结果。
